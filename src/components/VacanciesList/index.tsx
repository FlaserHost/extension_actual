...
const VacanciesList: FC<VacanciesListProps> = ({ vacancies, requestParams, setShowLoader, setConnected, getVacancyList }) => {
    ...
    const path = {serverUrl, databaseUrl, token, portUrl};
                    ...
                    fetch(`${serverUrl + (databaseUrl ? '/' + databaseUrl : '') + (portUrl ? ':' + portUrl : '')}/hs/extension/candidates/candidates`, {
                        ...
                        .then(res => {
                            if (res.status === 200) {
                                ...
                                res.json().then(item => {
                                    if (item.error_text !== undefined) {
                                        setShowError({ show: true, closeText: 'Закрыть', title: item.error_head, text: item.error_text });
                                    }
                                })
        ...
        <div className={styles.wrapper}>
            ...
            {showError.show ?
                <Error setDouble={
                    ...
                    text={showError?.text}
                    doubleData={showError?.doubleData}
                    requestParams={showError?.requestParams}
                    setConnected={setConnected}
                    setShowLoader={setShowLoader}
                /> : ''
            }
